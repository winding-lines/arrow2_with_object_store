...
pub async fn parallel_read(
    ...
    let length = {
        ...
        locked_store
            .head(&path)
            .await
            .map_err(|e| format!("Error in head() {}", e.to_string()))?
            .size
    };
    ...
    let mut reader = reader_factory()
        .await
        .map_err(|e| format!("reader_factory {}", e.to_string()))?;

    let metadata = read::read_metadata_async(&mut reader)
        .await
        .map_err(|e| format!("read metadata async {}", e.to_string()))?;

    let schema =
        read::infer_schema(&metadata).map_err(|e| format!("infer schema {}", e.to_string()))?;
    ...
    let chunks = chunks
        ...
        .map_err(|e| format!("row group deserializer {}", e.to_string()))?;
